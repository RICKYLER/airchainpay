...
async function deployTokenContractToLiskSepolia() {
  console.log("🌐 AirChainPayToken - Lisk Sepolia Testnet Deployment");
  console.log("====================================================");
  ...
  try {
    ...
    const deploymentFile = path.join(deploymentsDir, "lisk_sepolia_airchainpaytoken.json");
    ...
    const existingIndex = masterData.deployments.findIndex(d => d.network === "lisk_sepolia" && d.contractName === "AirChainPayToken");
    if (existingIndex >= 0) {
...
// Handle errors
deployTokenContractToLiskSepolia()
  .then(() => process.exit(0))
  .catch((error) => {
