...
async function checkCoreTestnetDeployment() {
  console.log("🔍 AirChainPay - Core Testnet Deployment Check");
  console.log("==============================================");
  ...
  try {
    ...
    if (!fs.existsSync(deploymentFile)) {
      console.log("❌ No deployment found for Core Testnet");
      console.log("💡 To deploy, run: npx hardhat run scripts/deploy-core-testnet.js --network core_testnet");
      return;
    }
    ...
    console.log(`\n💰 Deployer balance: ${ethers.formatEther(balance)} tCORE2`);
    
    // Check contract balance
    const contractBalance = await provider.getBalance(deploymentInfo.contractAddress);
    console.log(`💰 Contract balance: ${ethers.formatEther(contractBalance)} tCORE2`);
    ...
    console.log(`   Network Info: https://chainlist.org/chain/1114`);
