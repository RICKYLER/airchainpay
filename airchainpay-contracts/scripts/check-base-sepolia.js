...
async function checkBaseSepoliaDeployment() {
  console.log("🔍 AirChainPay - Base Sepolia Testnet Deployment Check");
  console.log("=====================================================");
  
  try {
    // Check if deployment file exists
    const deploymentFile = path.join(__dirname, "../deployments/base_sepolia.json");
    
    if (!fs.existsSync(deploymentFile)) {
      console.log("❌ No deployment found for Base Sepolia Testnet");
      console.log("💡 To deploy, run: npx hardhat run scripts/deploy-base-sepolia.js --network base_sepolia");
      return;
    }
    ...
    console.log(`   Network Info: https://chainlist.org/chain/84532`);
