...
async function checkHoleskyDeployment() {
  console.log("🔍 AirChainPay - Ethereum Holesky Testnet Deployment Check");
  console.log("==========================================================");
  
  try {
    // Check if deployment file exists
    const deploymentFile = path.join(__dirname, "../deployments/holesky.json");
    
    if (!fs.existsSync(deploymentFile)) {
      console.log("❌ No deployment found for Ethereum Holesky Testnet");
      console.log("💡 To deploy, run: npx hardhat run scripts/deploy-holesky.js --network holesky");
      return;
    }
    ...
    console.log(`   Network Info: https://chainlist.org/chain/17000`);
